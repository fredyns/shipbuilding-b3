<div>
    <div>
        @can('create', App\Models\DailyActivity::class)
            <button class="button" wire:click="newDailyActivity">
                <i class="mr-1 icon ion-md-add text-primary"></i>
                @lang('crud.common.new')
            </button>
        @endcan @can('delete-any', App\Models\DailyActivity::class)
            <button
    ...
    <x-modal wire:model="showingModalView">
                ...
                <div class="flex flex-wrap">
                    <div class="mb-4 w-full">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.daily_report_activities.inputs.name')
                        </h5>
                        <span> {{ $dailyActivity->name ?? '-' }} </span>
                    </div>
                    <div class="mb-4 w-full">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.daily_report_activities.inputs.pic')
                        </h5>
                        <span> {{ $dailyActivity->pic ?? '-' }} </span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="px-6 py-4 bg-gray-50 flex justify-between">
            ...
            @can('update', $dailyActivity)
                <button
                    type="button"
                    class="button mr-1"
                    wire:click="editDailyActivity('{{ $dailyActivity->id }}')"
                >
                    ...
                    <td class="px-4 py-3 text-right" style="width: 134px;">
                        ...
                        >
                            <button
                                type="button"
                                class="button mr-1"
                                wire:click="viewDailyActivity('{{ $dailyActivity->id }}')"
                            >
